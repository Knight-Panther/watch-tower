...
type ScheduleManagerProps = {
  ...
  onSectorIntervalChange: (id: string, value: string) => void;
  onSaveSectorInterval: (id: string) => void;
};
...
export default function ScheduleManager({
  ...
  onSectorIntervalChange,
  onSaveSectorInterval,
  ...
  return (
    <>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        <h2 className="text-lg font-semibold">Sector overrides</h2>
          ...
          {sectors.map((sector) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <input
                  value={sectorIntervalDrafts[sector.id] ?? String(sector.ingest_interval_minutes ?? "")}
                  onChange={(event) => onSectorIntervalChange(sector.id, event.target.value)}
                  placeholder="Interval (min)"
                  className="w-32 rounded-full border border-slate-700 bg-slate-950 px-3 py-1 text-xs text-slate-200"
                />
                <button
                  onClick={() => onSaveSectorInterval(sector.id)}
